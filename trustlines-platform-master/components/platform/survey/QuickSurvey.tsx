...
// Prospect. The handwritten Note is folded into the same composed notes text that already
// lands in the CRM's Notes/Timing display (see lib/marketing/opportunityEngine.ts
// composedNotes() + the "Why this stage"/"Timing" rows in OpportunityQuickView.tsx).
...
function buildSubmissionPayload(data: SurveyData, submissionToken: string, consentTextVersion: string) {
  ...
  return {
    // No company-name field on this short form — the Prospect is identified by the
    // person's own name instead of an organization name (GeneralSurvey.tsx uses the
    // opposite, since it collects a company name and this one doesn't).
...
export function QuickSurvey({ campaignSlug, consentTextVersion }: { campaignSlug: string; consentTextVersion: string }) {
  ...
  const iconByStep = [Store, Building2, Clock3];
      ...
      <div className="gs-survey-shell">
        <aside className="gs-brand-rail">
          ...
          <div className="gs-rail-copy">
            ...
            <p>A quick project brief that lands straight in our team&apos;s pipeline — just the essentials.</p>
          </div>
        ...
        <main className="gs-survey-main" ref={mainRef}>
          ...
          <div className="gs-progress-wrap">
            <div className="gs-progress-meta"><span>Project journey</span><span>Step {displayStep} of 3</span></div>
            <div className="gs-progress-track"><span style={{ width: `${(displayStep / 3) * 100}%` }} /></div>
          </div>
