...
import { getCampaign, trashCampaign, CampaignError } from '@/lib/marketing/campaigns';
...
export async function POST(_req: NextRequest, { params }: Params) {
  ...
  try {
    await trashCampaign(admin, id);
    await logAudit({ actorId: user.id, action: 'campaign.trashed', resource: `campaign:${id}`, newValue: { name: campaign.name } });
