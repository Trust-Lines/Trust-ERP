...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string; taskId: string }> }) {
  ...
  const { error } = await admin.from('lead_tasks').update(patch).eq('id', taskId).eq('potential_id', id);
  ...
  if (patch.assignee_id && patch.assignee_id !== user.id) {
    ...
    await notifyUser(admin, { userId: patch.assignee_id as string, title: 'You were assigned a task', body: title });
  }
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string; taskId: string }> }) {
  ...
  const { error } = await admin.from('lead_tasks').delete().eq('id', taskId).eq('potential_id', id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ ok: true });
}
