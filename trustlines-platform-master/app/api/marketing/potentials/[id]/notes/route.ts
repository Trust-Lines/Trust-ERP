...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  if (!pot?.need_id) return NextResponse.json({ notes: [] });

  const { data, error } = await admin.from('need_notes')
    .select(NOTE_COLS).eq('need_id', pot.need_id)
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  if (image && image.size > 0) {
    ...
    try {
      ...
      console.error('[potentials/notes POST image]', msg);
  ...
  await logAudit({ actorId: user.id, action: 'potential.note_posted', resource: `need_notes:${note.id}`, newValue: { potential_id: id } });
