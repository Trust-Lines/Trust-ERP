...
export async function GET(req: NextRequest, { params }: Params) {
  ...
  if (!opp?.need_id) return NextResponse.json({ error: 'Not found' }, { status: 404 });

  const path = req.nextUrl.searchParams.get('path') ?? '';
  if (!path) return NextResponse.json({ error: 'path required' }, { status: 400 });

  const [{ data: fileMatch }, { data: noteMatch }] = await Promise.all([
    admin.from('need_files').select('id').eq('need_id', opp.need_id).eq('dropbox_path', path).maybeSingle(),
    admin.from('need_notes').select('id').eq('need_id', opp.need_id).eq('image_path', path).maybeSingle(),
  ]);
